...
exports.LargeFirstSelector = void 0;
...
const pickAdditionalLargestUtxo = ({ utxoRemaining, utxoSelected }) => {
    ...
    const sorted = utxoRemaining.sort(([, a], [, b]) => (0, util_1.sortByCoins)(a, b));
    ...
    async select(params) {
        ...
        return {
            ...
            selection: {
                change,
                fee,
                inputs: new Set([...finalSelection].sort(util_1.sortUtxoByTxIn)),
                outputs
            }
        };
    }
}
exports.LargeFirstSelector = LargeFirstSelector;
_LargeFirstSelector_props = new WeakMap(), _LargeFirstSelector_instances = new WeakSet(), _LargeFirstSelector_aggregateLovelace = function _LargeFirstSelector_aggregateLovelace(inputs, outputs, implicit) {
    const utxoAda = (0, util_1.getCoinQuantity)(inputs);
    const outputAda = (0, util_1.getCoinQuantity)(outputs);
    ...
    const outputsMap = (0, core_1.coalesceValueQuantities)(outputs).assets ?? new Map();
    const utxoMap = (0, core_1.coalesceValueQuantities)(inputs).assets;
    ...
    let required = (0, util_1.addTokenMaps)(outputsMap, negMint);
    required = (0, util_1.subtractTokenMaps)(required, posMint) ?? new Map();
    const available = (0, util_1.addTokenMaps)(utxoMap, posMint) ?? new Map();
    ...
    const inputVals = (0, util_1.toValues)([...inputs]);
    const outputVals = (0, util_1.toValues)([...outputs]);
    ...
    const changeAssets = (0, util_1.subtractTokenMaps)(available, required);
    if (inputs.size === 0 || changeAda < 0n || (0, util_1.hasNegativeAssetValue)(changeAssets))
    ...
    for (const [assetId, requiredQuantity] of requiredAssets) {
        const candidates = allAvailable
            ...
            .sort(([, a], [, b]) => (0, util_1.sortByAssetQuantity)(assetId)(a, b));
        let accumulated = 0n;
    ...
    const adaCandidates = allAvailable
        ...
        .sort(([, a], [, b]) => (0, util_1.sortByCoins)(a, b));
    let adaAccumulated = (0, util_1.getCoinQuantity)((0, util_1.toValues)([...result])) + implicitCoinInput;
    ...
    const utxoRemainingArr = allAvailable
        ...
        .sort(([, a], [, b]) => (0, util_1.sortByCoins)(a, b));
    const outputValues = (0, util_1.toValues)([...outputs]);
    ...
    const uniqueTxAssetIDs = (0, uniq_js_1.default)([...requiredAssets.keys(), ...mintMap.keys()]);
    const { implicitTokensInput, implicitTokensSpend } = (0, util_1.mintToImplicitTokens)(mintMap);
    const implicitTokens = {
        input: (assetId) => implicitTokensInput.get(assetId) || 0n,
        spend: (assetId) => implicitTokensSpend.get(assetId) || 0n
    };
    const { change, fee, inputs: finalInputs } = await (0, change_1.computeChangeAndAdjustForFee)({
